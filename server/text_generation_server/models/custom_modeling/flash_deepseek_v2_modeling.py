...
class BlockSparseMoE(nn.Module):
    def __init__(self, prefix, config: DeepseekV2Config, weights):
        ...
        if config.ep_size != 1:
            raise ValueError(
                f"Currently only ep_size == 1 is supported by BlockSparseMoE, was {config.ep_size}"
            )
...
class DenseMoE(nn.Module):
    def __init__(self, prefix: str, config: DeepseekV2Config, weights: Weights):
        ...
        if config.ep_size != 1:
            raise ValueError(
                f"Currently only ep_size == 1 is supported by DenseMoE, was {config.ep_size}"
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # out = self.moe_infer_cpu(x, topk_ids, topk_weights) * self.routed_scaling_factor
